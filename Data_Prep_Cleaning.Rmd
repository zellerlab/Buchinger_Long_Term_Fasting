...
completed_substrate_annotations <- read_xlsx(here("Other_Required_Data", "20250219_Table_S1_incl_dbCAN3_annotations.xlsx"))
...
## Load and prepare fecal metabolomics data
```{r}
## Only patientID 11 has just 1 sample, others all before-after
faPeaks <- read.csv(here("Fecal_Metabolite_Data","Gen_Fecal_Neg_All_Peaks.csv"))
faMeta <- read.csv(here("Fecal_Metabolite_Data","Gen_Fecal_All_Meta.csv"))

faPeaks <- faPeaks %>% select(compound, starts_with("gen"), starts_with("X"))
faPeaks <- reshape2::melt(faPeaks)

colnames(faPeaks) <- c("compound", "sampleId", "IC")

faPeaks <- merge(faPeaks, faMeta)
faPeaks <- faPeaks %>% group_by(compound) %>% mutate(IC = ifelse(IC == 0, min(IC[IC>0]), IC))
...
faPeaks$timepoint <- factor(faPeaks$timepoint, levels = c("pre", "post"))

faPeaks <- faPeaks %>% mutate(ticNormIC = IC/normFactor)

## To have it corresponding to table S7, post is "end"
Table_S8 <- faPeaks 
Table_S8 <- Table_S8 %>% mutate(timepoint = recode(timepoint, pre  = "baseline", post = "end"))
Table_S8$Study_Patient_Timepoint <- paste("GEN", Table_S8$patientId, Table_S8$timepoint, sep = "_")

## In order to have all the metabolites have a unique name
Table_S8 <- Table_S8 %>%
  group_by(Study_Patient_Timepoint) %>%
  mutate(compound = make.unique(as.character(compound))) %>%
  ungroup()

Table_S8 <- Table_S8 %>% select(compound, normLogIC, Study_Patient_Timepoint) %>% pivot_wider(names_from = compound, values_from = normLogIC) %>% as.data.frame()
#write_xlsx(Table_S8, here("Supplementary_Tables","Table_S8.xlsx"))
```
