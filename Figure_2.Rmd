...
Fig_2A <- plot_ordination(Mesnage_2023_tax_ps_rel_abun_4, pcoa_info_ltfu, color="Timepoint") + scale_color_manual(values=c('#3B6FB6','#D41645','grey','orange')) + theme_publication() + theme(legend.position = "none") + xlab("PC1 [10.2%]") + ylab("PC2 [8.0%]")
```
...
remove_patients <- microbiome::meta(Mesnage_2023_tax_ps_rel_abun_ltfu) %>% group_by(Study_Patient) %>% filter("Before" %in% Timepoint) %>% ungroup()
Mesnage_2023_tax_ps_rel_abun_ltfu_bc <- Mesnage_2023_tax_ps_rel_abun_ltfu %>% ps_filter(Study_Patient %in% remove_patients$Study_Patient)
