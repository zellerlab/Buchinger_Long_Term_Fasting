...
## Compare serum metaB to fecal metaB. Only retain fecal metabolites that are also in serum metaB
...
ggplot(result_serum_fecal_comparison) +
  theme_publication() + 
  ...
  ylab("Enrichment effect size feces") +
  xlab("Enrichment effect size serum") +
  #geom_abline(intercept = 0, slope = 1) +
  ggrepel::geom_text_repel(data = result_serum_fecal_comparison %>% filter(str_detect(Significant, "Both Significant") | estimate_serum > 0.4 | estimate_feces > 0.4),
  aes(label = matched_prefix), max.overlaps = 10, size = 2.5, color = "black") +
  xlim(-0.6, 0.6) +
  ylim(-1.5, 1.5) +
...
## Compare serum to fecal metaB
```{r}
## Some chatgpt magic to do matching based on the strings. Here I want to only retain metabolites from the serum metaB that also occur in fecal metaB
GenStats_combined_Time_serum_fecal_comparison <- GenStats_combined_Time %>% filter(term == "timepointend") %>% filter(str_detect(compound, paste0("^(", paste0(faPeaksStatsMix$compound, collapse = "|"), ")(_|$)")))
